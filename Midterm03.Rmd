...
A Gaussian likelihood has two parameters, the mean and variance, also called $\mu$ and $\sigma$. Both of these parameters can take on any real value on the number line. The mean or $\mu$ dictates where the symmetrica gaussian distribution is centered. The variance or $\sigma^2$ dictates how high the peak is compared to how wide/large the tails are. A lower variance corresponds to a tigher/higher peak. In this problem we will hold $\sigma$ fixed and "known".
